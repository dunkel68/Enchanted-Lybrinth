...
public:
    ...
    std::vector<std::pair<int, int>> shortestPath(int startRow, int startCol, int endRow, int endCol, Maze maze) {
        if (startRow < 0 || startRow >= nodes.size() || startCol < 0 || startCol >= nodes[0].size() ||
            endRow < 0 || endRow >= nodes.size() || endCol < 0 || endCol >= nodes[0].size()) {
            std::cerr << "Error: Invalid start or end coordinates." << std::endl;
            return std::vector<std::pair<int, int>>(); // Invalid start or end coordinates
        }
        ...
        while (!pq.empty()) {
            ...
            if (current->getRow() == endRow && current->getCol() == endCol) {
                ...
                while (current != nullptr) {
                    shortestPath.emplace_back(current->getRow(), current->getCol());
            ...
            if (!current->isVisited()) {
                ...
                for (Node* neighbor : current->getNeighbors()) {
                    ...
                    if (neighbor->getRow() >= 3 && neighbor->getCol() >= 3 && !maze.isWall(neighbor->getRow(), neighbor->getCol()) && !neighbor->isVisited()) {
